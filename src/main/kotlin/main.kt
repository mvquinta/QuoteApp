...
fun main(){
    ...
    do {
        ...
        val myInput = startQuoteApp()
        val myInputCatFilter = (".$myInput.")
        ...
        fun printCategoryQuote(selectCategory: String) {
            ...
            val containsCategory = readQuotes.filter { it.contains(myInput.toLowerCase()) }
        ...
        fun printTotalRandomQuote() {
            ...
            val randomIntCat: Int = (1 until categoriesListSize).random() //creates a random int based on list size. Range starts in 1 to avoid choosing random category that, in my txt file, is first category (indices 0) and is empty.
        ...
        //This should be my final code if I manage to put start of program inside a function
        if(myInputCatFilter != ".RANDOM.") {
            printCategoryQuote(myInput)
        } else {
        ...
        if (anotherQuote != "yes" && anotherQuote != "Yes" && anotherQuote != "y" && anotherQuote != "Y" && anotherQuote != "YES") {println("Hope you liked your quote. Thank you!")}
    } while (anotherQuote == "yes" || anotherQuote == "Yes" || anotherQuote == "y" || anotherQuote == "Y" || anotherQuote == "YES")
}